...
func TestRpcSend(t *testing.T) {
	...
	bc, err := logic.CreateBlockchain(senderWallet.GetAddress(), store, pow, 128, scManager, 1000000)
	if err != nil {
		panic(err)
	}
...
func createRpcTestContext(startPortOffset uint32) (*RpcTestContext, error) {
	...
	scManager := vm.NewV8EngineManager(core.Address{})
	bc, err := logic.CreateBlockchain(wallet.GetAddress(), context.store, context.consensus, 128, scManager, 1000000)
	if err != nil {
		context.destroyContext()
		panic(err)
	}
