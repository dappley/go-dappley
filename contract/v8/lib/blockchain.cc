...
void NewBlockchainInstance(Isolate *isolate, Local<Context> context, void *handler) {
    ...
    blockTpl->Set(String::NewFromUtf8(isolate, "getNodeAddress"), FunctionTemplate::New(isolate, GetNodeAddressCallback),
                  static_cast<PropertyAttribute>(PropertyAttribute::DontDelete | PropertyAttribute::ReadOnly));
...
void GetNodeAddressCallback(const FunctionCallbackInfo<Value> &info) {
    ...
    if (info.Length() != 0) {
        isolate->ThrowException(String::NewFromUtf8(isolate, "Blockchain.GetNodeAddressCallback() does not require any argument"));
        return;
    }
