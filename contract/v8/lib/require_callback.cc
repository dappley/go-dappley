...
void RequireCallback(const v8::FunctionCallbackInfo<v8::Value> &info) {
    ...
    if (info.Length() == 0) {
        isolate->ThrowException(Exception::Error(String::NewFromUtf8(isolate, "require missing path")));
        return;
    }

    Local<Value> path = info[0];
    if (!path->IsString()) {
        isolate->ThrowException(Exception::Error(String::NewFromUtf8(isolate, "require path must be string")));
        return;
    }

    String::Utf8Value filename(isolate, path);
    if (filename.length() >= MAX_PATH_LEN) {
        isolate->ThrowException(Exception::Error(String::NewFromUtf8(isolate, "require path length more")));
        return;
    }
    char *abPath = NULL;
    if (strcmp(*filename, LIB_WHITE)) {  // if needed, check array instead.
        ...
        if (abPath == NULL) {
            isolate->ThrowException(Exception::Error(String::NewFromUtf8(isolate, "require path is invalid absolutepath")));
            return;
        }
        char curPath[MAX_VERSIONED_PATH_LEN] = {0};
        if (curPath[0] == 0x00 && !getCurAbsolute(curPath, MAX_VERSIONED_PATH_LEN)) {
            isolate->ThrowException(Exception::Error(String::NewFromUtf8(isolate, "invalid cwd absolutepath")));
            free(abPath);
            return;
        }
        int curLen = strlen(curPath);
        if (strncmp(abPath, curPath, curLen) != 0) {
            isolate->ThrowException(Exception::Error(String::NewFromUtf8(isolate, "require path is not in lib")));
            free(abPath);
            return;
        }
    ...
    MaybeLocal<Script> script = Script::Compile(context, String::NewFromUtf8(isolate, data), &sourceSrcOrigin);
    if (!script.IsEmpty()) {
