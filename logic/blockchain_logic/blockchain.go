...
func (bc *Blockchain) GetUtxoCache() *core.UTXOCache {
	return bc.utxoCache
}
...
func (bc *Blockchain) FindTXOutput(in core.TXInput) (core.TXOutput, error) {
	db := bc.db
	vout, err := core.GetTxOutput(in, db)
	if err != nil {
		return core.TXOutput{}, err
	}
	return vout, err
}
