...
func TestSend(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			bc, pow := createBlockchain(senderAccount.GetKeyPair().GenerateAddress(), store)
...
func TestSendToInvalidAddress(t *testing.T) {
	...
	_, _, err = Send(account1, account.NewAddress(InvalidAddress), transferAmount, tip, common.NewAmount(0), common.NewAmount(0), "", bc, node)
	assert.NotNil(t, err)
...
func TestBlockMsgRelaySingleMiner(t *testing.T) {
	...
	for i := 0; i < numOfNodes; i++ {
		...
		bc := core.CreateBlockchain(account.NewAddress(producerAddrs[0]), storage.NewRamStorage(), dpos, 128, nil, 100000)
		bcs = append(bcs, bc)
		pool := core.NewBlockPool(0)
...
func TestAddBalanceWithInvalidAddress(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			pool := core.NewBlockPool(0)
			node := network.FakeNodeWithPidAndAddr(pool, bc, "a", "b")
			_, _, err = SendFromMiner(account.NewAddress(tc.address), common.NewAmount(8), bc, node)
...
func TestSmartContractLocalStorage(t *testing.T) {
	...
	_, _, err = Send(senderAccount, account.NewAddress(""), common.NewAmount(1), common.NewAmount(0), common.NewAmount(10000), common.NewAmount(1), contract, bc, node)
	assert.Nil(t, err)
...
func createBlockchain(addr account.Address, db *storage.RamStorage) (*core.Blockchain, *consensus.ProofOfWork) {
...
func TestDoubleMint(t *testing.T) {
	...
	for i := 0; i < 2; i++ {
		...
		dpos := consensus.NewDPOS()
		dpos.SetDynasty(dynasty)
		bc := core.CreateBlockchain(account.NewAddress(validProducerAddr), storage.NewRamStorage(), dpos, 128, nil, 100000)
		pool := core.NewBlockPool(0)
		node := network.NewNode(bc, pool)
...
func TestSimultaneousSyncingAndBlockProducing(t *testing.T) {
	...
	bc := core.CreateBlockchain(account.NewAddress(genesisAddr), storage.NewRamStorage(), conss, 128, nil, 100000)
	...
	bc1 := core.CreateBlockchain(account.NewAddress(genesisAddr), storage.NewRamStorage(), dpos, 128, nil, 100000)
