...
// TxContractGen transaction, generated by contract execution
type TxContractGen struct {
	*transaction.Transaction
}
...
func NewTxDecorator(tx *transaction.Transaction) TxDecorator {
	...
	case transaction.TxTypeReward:
		return &TxReward{tx}
	case transaction.TxTypeContractGen:
		return &TxContractGen{tx}
	}
	return nil
}
...
func (tx *TxContractGen) Sign(privKey ecdsa.PrivateKey, prevUtxos []*utxo.UTXO) error {
	return nil
}

func (tx *TxContractGen) Verify(utxoIndex *lutxo.UTXOIndex, blockHeight uint64) error {
