...
void InitializeMath(FuncRandom random) { sRandom = random; }
...
void NewMathInstance(Isolate *isolate, Local<Context> context, void *handler) {
    ...
    mathTpl->Set(String::NewFromUtf8(isolate, "random"), FunctionTemplate::New(isolate, RandomCallback),
    ...
    context->Global()->DefineOwnProperty(context, String::NewFromUtf8(isolate, "math"), instance,
                                         static_cast<PropertyAttribute>(PropertyAttribute::DontDelete | PropertyAttribute::ReadOnly));
}
...
void RandomCallback(const FunctionCallbackInfo<Value> &info) {
    ...
    if (info.Length() != 1) {
        isolate->ThrowException(String::NewFromUtf8(isolate, "math.random() requires 1 argument"));
        return;
    }

    Local<Value> max = info[0];
    if (!max->IsNumber()) {
        isolate->ThrowException(String::NewFromUtf8(isolate, "input must be a number"));
        return;
    }
