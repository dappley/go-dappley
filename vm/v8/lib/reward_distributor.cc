...
void InitializeRewardDistributor(FuncRecordReward recordReward) { sRecordReward = recordReward; }
...
void NewRewardDistributorInstance(Isolate *isolate, Local<Context> context, void *handler) {
    ...
    rdTpl->Set(String::NewFromUtf8(isolate, "record"), FunctionTemplate::New(isolate, RecordRewardCallback),
    ...
    context->Global()->DefineOwnProperty(context, String::NewFromUtf8(isolate, "_native_reward"), instance,
                                         static_cast<PropertyAttribute>(PropertyAttribute::DontDelete | PropertyAttribute::ReadOnly));
}

// RecordRewardCallback
void RecordRewardCallback(const FunctionCallbackInfo<Value> &info) {
    ...
    if (info.Length() != 2) {
        isolate->ThrowException(String::NewFromUtf8(isolate, "Blockchain.verifyAddress() requires 2 arguments"));
        return;
    }
