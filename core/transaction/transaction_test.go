...
func TestTransaction_VerifyPublicKeyHash(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			expectedRes: false,
			expectedErr: errors.New("Transaction: prevUtxos not found"),
		},
		{
			...
			expectedRes: false,
			expectedErr: errors.New("public key not correct"),
		},
		{
			...
			expectedRes: false,
			expectedErr: errors.New("the pubic key hash does not match"),
		},
		{
			name: "contract",
			tx: &Transaction{
...
func TestTransaction_Verify(t *testing.T) {
	...
	tx.ID = (&txCopy).Hash()
	err = tx.Verify(utxos)
	assert.Equal(t, errors.New("Transaction: vout is invalid"), err)
...
func TestTransaction_CheckVinNum(t *testing.T) {
	...
	tx.Vin = append(tx.Vin, transactionbase.TXInput{})
	assert.Equal(t, errors.New("too many vin in a transaction"), tx.CheckVinNum())
}
